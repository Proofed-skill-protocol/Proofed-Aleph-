...
import { createChallenge, submitToChallenge, evaluateSubmission, getSubmission, parseFeedback, type Challenge } from '@/lib/genlayer/client';
...
interface Screen5Props {
  ...
  challengeId: string;
  dynamicChallenge?: Challenge | null;
...
export default function Screen5({ selTrack, githubUrl, walletAddress, challengeId, dynamicChallenge, onDone }: Screen5Props) {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        setP1('active');
        if (dynamicChallenge) {
          setChainError('Initiating AI dynamic challenge on GenLayer...');
          await createChallenge({
            challengeId: dynamicChallenge.id,
            title: dynamicChallenge.title,
            description: dynamicChallenge.description,
            rubric: dynamicChallenge.rubric,
            walletAddress,
          });
          setChainError(null);
        }
        
        setChainError('Submitting your exact repository to the GenLayer contract...');
        const submitTx = await submitToChallenge({ challengeId, githubUrl, walletAddress });
        if (!submitTx) throw new Error('Transaction rejected or failed by network.');
        setChainError(null);
        await sleep(800);
        setP1('done');
        ...
        setChainError('Awaiting GenLayer Intelligent Validators distributed consensus...');
        const [apiResult, evaluateTx] = await Promise.all([
          apiPromise,
          evaluateSubmission({ challengeId, walletAddress }),
        ]);

        if (!evaluateTx) throw new Error('On-chain evaluation transaction reverted.');

        // Try to read back the on-chain result
        let onChainScore: number | null = null;
        let onChainFeedback: any = null;
        
        await sleep(3000); // Allow blocks to confirm
        const sub = await getSubmission({ challengeId, submitterAddress: walletAddress });
        if (!sub?.has_evaluated) throw new Error('Intelligent evaluation process failed to post results to chain.');

        onChainScore    = sub.score;
        onChainFeedback = parseFeedback(sub.feedback);

        const base   = onChainScore;
        ...
        onDone({
          score:          onChainScore,
          strengths:      onChainFeedback?.strengths,
          improvements:   onChainFeedback?.improvements,
          summary:        onChainFeedback?.category_breakdown,
        ...
        setFatalError(err.message || 'Unknown network error occurred');
        setShowSpin(false);
      }
    };
    run();
  ...
  return (
    <div className="screen on">
      <div className="eval-wrap">
        ...
        {fatalError && (
          <div style={{ backgroundColor: 'rgba(255,50,50,0.1)', border: '1px solid var(--red)', padding: 15, borderRadius: 8, marginTop: 20 }}>
            <p style={{ color: 'var(--red)', fontWeight: 'bold', margin: '0 0 5px 0' }}>⛔ On-Chain Execution Failed</p>
            <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.85rem', margin: 0 }}>{fatalError}</p>
          </div>
        )}

        {apiError && !fatalError && <p style={{ color: 'orange', fontSize: '0.8rem', marginTop: 4 }}>⚠ Metadata check degraded.</p>}
        {chainError && !fatalError && <p style={{ color: 'var(--green)', fontSize: '0.8rem', marginTop: 4 }}>⚙ {chainError}</p>}
